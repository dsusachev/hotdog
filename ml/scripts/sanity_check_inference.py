...
def check(artifact_path: Path) -> None:
    ...
    print(
        f"model_version={p.model_version}, device={p.device}, "
    ...
    for i in range(n):
        ...
        assert (
            abs(pred["confidence"] - float(batch_probs[i, pred["category_id"]])) < 1e-5
        )
    ...
    print("[OK] predict(top_k=3) matches predict_all + invariants hold")
    ...
    print("[OK] threshold=0.0 -> never unknown")
    ...
    print("[OK] threshold=0.9999 -> is_unknown=True, prediction=None, top_k present")

    # k=1 degenerate case
    out_k1 = p.predict(raw_ds[0][0], top_k=1)
    assert len(out_k1["top_k"]) == 1
    assert out_k1["top_k"][0] == out_k1["prediction"]
    print("[OK] top_k=1 is a degenerate case of top-k")
    ...
    assert (
        out_pil["prediction"]["category_id"] == out_bytes["prediction"]["category_id"]
    ), "PIL and bytes inputs disagree on top-1"
    print("[OK] PIL / bytes inputs interchangeable")
