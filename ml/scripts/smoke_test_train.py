...
from src.dataset import NUM_COARSE_CLASSES, GroceryStoreDataset  # noqa: E402
...
def overfit_one_batch(device: torch.device) -> None:
    ...
    print(
        f"loss[0]={losses[0]:.3f}, loss[10]={losses[10]:.3f}, loss[-1]={losses[-1]:.3f}"
    )
...
def short_training_run() -> None:
    ...
    config = TrainConfig(
        ...
        stages=[
            StageConfig(
                "feature_extraction", lr=1e-3, epochs=2, backbone_trainable=False
            ),
            StageConfig(
                "fine_tuning",
                lr=1e-4,
    ...
    try:
        result = train_two_stage(
            model, config.backbone, train_loader, val_loader, config, run_dir
        )
